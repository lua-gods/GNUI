...
function Box.new(parent)
	...
	if cfg.debug_mode then
		new.debugBox = Sprite.new():setModelpart(new.ModelPart):setTexture(debugTex):setRenderType(
				 "CUTOUT_EMISSIVE_SOLID"):setBorderThickness(3, 3, 3, 3):setScale(cfg.debug_scale)
			 :excludeMiddle(true):setDepthOffset(-0.1)
...
function Box:setText(text)
	...
	for i = 1, #ft, 1 do
		local bt = ft[i]
		self.TextLengths[i] = client.getTextWidth("|" ..
