...
local s = require("./sprite") ---@type GNUI.Sprite
local ca = require("./prims/canvas") ---@type GNUI.Canvas
local bx = require("./prims/box") ---@type GNUI.Box
...
if host:isHost() then
	...
	---@param pitch number?
	---@param volume number?
	function playUISound(sound, pitch, volume)
		sounds[sound]:pos(client:getCameraPos():add(client:getCameraDir())):pitch(pitch or 1):volume(
		volume or 1):attenuation(9999):play()
	end
else
